...
def prepare_data(base_path: str, out_dir: str, sample_rows: int = 0):
    ...
    # AFTER (fixed):
    ratings_before = len(ratings)  # ← ADD THIS LINE
    print(f"Before filtering: {ratings_before} rows")
    ratings = ratings[
        (ratings['user_idx'] >= 0) & 
        (ratings['user_idx'] < n_users) &
        (ratings['item_idx'] >= 0) &
        (ratings['item_idx'] < n_items)
    ]
    ratings_after = len(ratings)
    print(f"After filtering: {ratings_after} rows (kept {100*ratings_after/ratings_before:.1f}%)")
    ...
    probe = probe[
        (probe['user_idx'] >= 0) & 
        (probe['user_idx'] < n_users) &
        (probe['item_idx'] >= 0) &
        (probe['item_idx'] < n_items)
    ]
    probe.to_csv(out_dir / 'probe_sample.csv', index=False)
    ...
    qual = qual[
        (qual['user_idx'] >= 0) & 
        (qual['user_idx'] < n_users) &
        (qual['item_idx'] >= 0) &
        (qual['item_idx'] < n_items)
    ]
    qual.to_csv(out_dir / 'qual_sample.csv', index=False)
